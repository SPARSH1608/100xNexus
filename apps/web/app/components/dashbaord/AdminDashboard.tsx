...
export default function AdminDashboard() {
    ...
    const activeContestsCount = useMemo(() =>
        contests.filter(c => c.status === "Active" || new Date(c.startTime) <= new Date() && new Date(c.endTime) > new Date()).length,
        [contests]
    )
    const totalStudents = useMemo(() => users.length, [users])

    // Sort users by ID assuming newer users have higher IDs
    const newStudents = useMemo(() => [...users].slice(-5).reverse(), [users]);

    // Filter active/live contests for the list
    const liveBattles = useMemo(() => contests.filter(c => c.status === "Active" || c.isOpenAll).slice(0, 4), [contests]);
                ...
                <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 pt-24 md:pt-12 md:pl-24 pb-12 relative z-10 text-white">
                    <div className="flex flex-col md:flex-row items-center justify-between mb-10 gap-6">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="h-12 w-96 bg-white/5 rounded-full animate-pulse" />
                            <div className="h-12 w-24 bg-white/5 rounded-full animate-pulse" />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                        <div className="xl:col-span-8 flex flex-col gap-8">
                            <div className="bg-[#111] rounded-[2.5rem] p-8 h-64 animate-pulse" />
                            <div className="bg-[#111] rounded-[2.5rem] p-8 h-32 animate-pulse" />
                            <div className="bg-[#111] rounded-[2.5rem] p-8 h-96 animate-pulse" />
                        </div>
